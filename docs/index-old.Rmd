---
title: "Avoir une présence en ligne: Outils pour la diffusion rapide et reproductible de la recherche - Having an online presence: Tools for reproducible and rapid dissemination of research"
subtitle: "Atelier en Calcul Statistiques / Statistical Computing Workshop CSSC 2018"
author: "Sahir Rai Bhatnagar (https://sahirbhatnagar.com, \\@syfi_24)"
date: "`r Sys.Date()`"
output:
  html_document:
    theme: flatly
...
```{r fig-nocap-margin-first, fig.margin=TRUE, fig.width=4, fig.height=4, cache=TRUE, echo=FALSE, eval=FALSE}
library(ggplot2)
ggplot(mtcars, aes(wt, mpg)) + geom_point(size=3, aes(colour=factor(cyl))) + theme(legend.position="none")
```

`r newthought('Please go to the following website to vote:')`
...
# Setting up a website using Blogdown

## Create 2 new GitHub Repositories 
...
```{r cssc-logo, fig.margin = TRUE}
knitr::include_graphics(path = "assets/img/cssc-logo.png")
```

![](assets/img/cssc-logo.png)
